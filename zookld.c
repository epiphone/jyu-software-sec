...
pid_t launch_svc(CONF *conf, const char *name)
{
    ...
    if ((dir = NCONF_get_string(conf, name, "dir")))
    {
        /* chroot into dir */
     	if (chdir(dir))
	        err(1, "cannot chdir into %s", dir);
     	if (chroot(dir))
            err(1, "cannot chroot into %s", dir);
	    warnx("chrooted into %s", dir);
    }
    ...
    if (NCONF_get_number_e(conf, name, "gid", &gid))
    {
        ...
        if (setresgid(gid, gid, gid))
            err(1, "cannot setresgid to %ld", gid);
        warnx("setgid %ld", gid);
    }
    ...
    if ((groups = NCONF_get_string(conf, name, "extra_gids")))
    {
        ...
        if (setgroups(ngids, gids))
            err(1, "cannot setgroups to %s", groups);
        for (i = 0; i < ngids; i++)
    ...
    if (NCONF_get_number_e(conf, name, "uid", &uid))
    {
        ...
        if (setresuid(uid, uid, uid))
            err(1, "cannot setresuid to %ld", uid);
        warnx("setuid %ld", uid);
    }
