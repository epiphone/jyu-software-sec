...
def concolic_test(testfunc, maxiter = 100, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    for i, (c, caller) in enumerate(zip(cur_path_constr, cur_path_constr_callers)):
      opposite_constr = sym_and(*[sym_not(c)] + cur_path_constr[:i])

      if opposite_constr not in checked:
        checked.add(opposite_constr)

        (ok, model) = fork_and_check(opposite_constr)
        if ok == z3.sat:
