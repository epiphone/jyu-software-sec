...
def test_stuff():
  ...
  users1 = pdb.query(zoobar.zoodb.Person).all()
  balance1 = sum([p.zoobars for p in users1])
  pdb.commit()
  ...
  # If no new users are registered, the sum of Zoobar balances in all accounts
  # should remain the same before and after every request. (That is, zoobars
  # should never be created out of thin air.)
  users2 = pdb.query(zoobar.zoodb.Person).all()
  balance2 = sum([p.zoobars for p in users2])
  if set(p.username for p in users1) == set(p.username for p in users2):
    if balance1 != balance2:
      report_balance_mismatch()

  ## Detect zoobar theft.
  ## When detected, call report_zoobar_theft()
  # If a user u does not issue any requests to Zoobar, u 's Zoobar balance
  # should not shrink. (That is, it should be impossible for requests by one
  # user to steal another user's zoobars.)
  transfers = tdb.query(zoobar.zoodb.Transfer).all()
  for user in users2:
    sent = [t for t in transfers if t.sender == user.username]
    user_after = [u for u in users2 if u.username == user.username][0]
    if not sent and user_after.zoobars < 10:
      report_zoobar_theft()
